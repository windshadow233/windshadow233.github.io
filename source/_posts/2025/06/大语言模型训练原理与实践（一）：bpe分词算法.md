...
- 上策：以词为编码编码单位，可以保留完整的语义信息，例如：`internationalization`这个词整体作为一个token，语义信息丰富，看上去省事省心一步到位。然而，问题来了——如果是`internationalizations`呢？多了个 s，抱歉，模型词典里没有，不认识。由此可见，词级编码面临一个大bug：**词表巨大、组合爆炸、扩展性差**。你得穷尽所有单词的各种变形、拼写版本……搞得像字典编辑部天天加班。
...
这就是大模型训练的第二块拼图——笔者将在[下一篇文章](/blog/12690/)中进行讨论。
