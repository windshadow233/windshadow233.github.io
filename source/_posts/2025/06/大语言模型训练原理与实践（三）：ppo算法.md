...
设想我们的 Agent 当前处在一个水深火热的状态 $$s_t$$ ~~（水和火怎么来的你先别管）~~，在这个糟糕的状态下，无论这个 Agent 采取哪个动作 $$a$$，价值函数 $$Q(a\mid s_t)$$​ 都是负的，由刚刚**重要性采样**部分得出的结论，我们发现对于每个动作都要降低它被取到的概率，~~这不就是摆烂么~~。难道对于 Agent 而言，原地摆烂才是最优解？
